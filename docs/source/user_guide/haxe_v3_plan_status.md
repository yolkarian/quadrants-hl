...
`QD_BUILD_HASHLINK_LEGACY_TESTS` remains opt-in; those suites cover many older runtime semantics but are not the v3 gate.

## Phase matrix

| Phase | Status | Implemented in-tree | Remaining gaps | Main coverage/docs |
| --- | --- | --- | --- | --- |
| 0. Branch cut and deletion strategy | Mostly complete | Branch is `haxe-binding-v3`; haxelib version is `0.3.0-breaking`; v3 docs and v3 smoke test exist; legacy HashLink runtime tests are opt-in. | Some compatibility docs and APIs still live in the main package. | `tests/hashlink/v3/Smoke.hx`, `haxe_api_v3.md`, `haxe_kernel_v3.md`, `haxe_migration_v2_to_v3.md`, `haxe_capabilities.md`. |
| 1. Public API reset | Mostly complete | `Context.create(...)`, `Tensor<T>`, `Field<T>`, `LayoutPolicy`, `Layout`, typed `Kernel.build(...)` wrappers, private arg writers, and typed `QKernelN` launch are the documented path. Public dynamic launch was removed from `Kernel`. | `KernelRaw` remains as the explicit low-level bridge boundary; `ContextOptions` fluent helpers still exist but are not the v3 recommendation. | v3 smoke; compile-fail dtype/arity/resource tests; `dynamic_boundaries.md`. |
| 2. Descriptor schema reset | Complete for v3 native lowering | Binary QDHL descriptors use version `3`; descriptors now include native-parsed TypeTable, ArgTable, ResourceTable, StructTable, and SpecTable sections. Native and Haxe launch decoding canonicalize parameter/resource/spec lowering from these tables before compiling or launching the KernelIr body; `DescriptorV2` and the old descriptor docs path have been removed. JSON metadata has matching header/type/arg/resource/spec/capability fields. | Golden-file descriptor coverage exists and should be expanded as new resource forms are added. | `TestDescriptorSnapshot.hx`, `tests/hashlink/descriptor/DescriptorGoldenSnapshot.hx`; `haxe_descriptor.md`. |
