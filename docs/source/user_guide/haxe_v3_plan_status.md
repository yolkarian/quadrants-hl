...
| 6. Tensor/Field/SNode/layout/host movement | Partial | Tensor host movement, capability queries for DLPack/external pointer interop, and direct `Field<T>` kernel params are covered by v3 smoke/semantic tests; mirror fallback path removed. | Field offset placement and true DLPack/external-pointer roundtrip require backend/native support not available in the current CPU HashLink gate and are skipped rather than faked. |
