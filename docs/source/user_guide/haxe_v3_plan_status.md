...
| 6. Tensor/Field/SNode/layout/host movement | Partial | Tensor host movement and direct `Field<T>` kernel params are covered by v3 smoke/semantic tests; mirror fallback path removed. | More placement/layout/offset/DLPack coverage needed. |
| 7. AD/Tape/custom gradient | Partial | Reverse grad, forward grad, validation kernel launch, read-after-write validation rejection, Tape backward/withLoss/FwdMode, pause/resume/clear, registered custom-gradient replacement through typed `launchTape`, and integer/bool no-grad errors are covered by v3 runtime semantic tests. | Dynamic while-loop reverse/validation AD still reports an explicit unsupported error on the current backend; full dynamic-loop gradient parity remains open. |
| 8. Streams/events/graph | Partial | `launchOn`, `launchGraph`, host `launchGraphWhile`, `launchGraphDoWhile` capability error, `Graph.parallel`, explicit `launchTapeOn` AD/stream rejection, stream event capability errors, and gated event record/wait/sync success path are covered. | CI CPU backend does not exercise a supported native stream-event backend; native graph do-while remains capability-gated where unsupported. |
| 9. Algorithms/per-thread linalg | Partial | reduce/min/scan/select/sort/sort-pairs/reduce-by-key and host Linalg deterministic checks are covered. | Linalg is host semantic implementation, not device per-thread intrinsic parity; Python golden and benchmark coverage still missing. |
| 10. Sparse/Mesh/Quant | Partial | Sparse build/matvec/solver-with-explicit-fallback/mmwrite, mesh relation/attribute traversal, quantized kernel params, and quant-float placement are covered. | COO/CSR import/export helpers, mesh reorder/index conversion, and broader saturation/rounding golden tests need expansion. |
...
No phase is marked Complete for behavior that is only smoke-covered or explicitly unsupported. Remaining work is listed as Partial where native/backend depth is still missing, especially dynamic-loop AD and supported-backend stream event/do-while graph coverage.
