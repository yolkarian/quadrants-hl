...
| `new Field<T>(ctx, shape)` / `new Field<T>(ctx)` plus `ctx.root...place(field)` / `FieldScalar.i32(ctx)` / `Field<T>.toTensor()` / `Field<T>.fromTensor(...)` | Typed SNode-backed field allocation, scalar/root placement, host access, tensor conversion, and tensor-to-field copies. Direct `Field<T>` kernel parameters bind the placed SNode; `field[i]`, scalar read/write, dynamic `append`/`length`, and pointer/bitmasked `isActive`/`activate`/`deactivate` lower to native field/SNode IR instead of mirror tensor copies. Passing a field to an ndarray-only raw launch path is rejected instead of mirrored. |
...
| `Kernel.build(ctx, macro (...typed...) -> { ... }, {helpers: [MyHelpers]})` / `QD.kernel(...)` / `Kernel.buildRaw(...)` / `Template.build(...)` | Build and JIT-compile a typed `QKernelN` wrapper. `Kernel.buildRaw` returns the explicit low-level `KernelRaw` boundary for bridge/internal work. All v3 `Kernel.build` parameters should be explicitly typed. |
| `Kernel.descriptorBytes(..., {helpers: [MyHelpers]})` / `Template.descriptorBytes(...)` | Return QDHL descriptor bytes for tests/tooling. |
| `KernelRaw.launchDynamic(args)`, `launchBuffer(buf)`, `launchGraphDynamic(args)`, `launchGraphWhileDynamic(...)`, `launchGraphDoWhileDynamic(...)` | Low-level heterogeneous launch surface. New code should use typed `QKernelN.launch(...)`, `launchOn(...)`, and typed graph helpers instead. |
| `QKernelN.launch(...)` return values / `grad()` / `forwardGrad()` / `validationKernel()` | Typed launch, scalar/tuple return decoding, and typed autodiff-mode kernel wrappers. |
...
| `quadrants.coverage.Coverage`, `quadrants.compat.Diagnostics`, `quadrants.descriptor.Descriptor`, `DescriptorV3` | Kernel build/launch/source-span coverage JSON artifacts, descriptor dumps/hashes, canonical descriptor v3 metadata, value info, and runtime health checks. |
...
| `VectorNdarray<T>`, `MatrixNdarray<T>`, `VectorField<T>`, `MatrixField<T>`, `StructField`, `quadrants.flatten.Flattened`, `quadrants.flatten.DataOriented` | First-class compound storage containers for flat AOS vector/matrix storage and SOA struct fields, plus flatten/data-oriented helpers for typed host containers and instance kernels. |
...
| typed `QKernelN.launch(...)` with anonymous-structure return type | Decode flattened struct returns back into nested Haxe object literals through the typed return decoder. |
...
Prefer `@:build(quadrants.macro.QdStruct.build())` with `StructTensor<S>` / `StructField<S>` for new code. `StructTensor<S>` kernel parameters support descriptor-expanded whole-value load-copy-store and direct scalar, vector/matrix lane, and nested member reads/writes. Host code that needs typed whole-value movement can use `Struct.value1/2/3/4(...)` and `readValue*/writeValue*` on compatibility containers; these facades preserve member value types without exposing `Dynamic`. The old `add("name", value)`, `member("name")`, `read("name", i)`, and `write("name", i, value)` methods are compatibility shims and are listed in the retained-`Dynamic` ledger.
...
The relation handle type encodes the valid source and target domains, so passing an edge to a vertex-to-face relation is a compile-time error. Kernels can accept `MeshRelation<From, To>` and `MeshAttribute<Element, Value>` parameters; relation handles lower through native mesh topology handles backed by SNode relation resources and attributes lower to field-backed resources, supporting `relation.size(i)`, `relation.get(i, j)`, `attribute.read(i)`, and `attribute.write(i, value)`.

`quadrants.quant` provides typed storage descriptors, a reference/debug quantized tensor with kernel parameter support, and native `quant_array` field placement for int/fixed specs:
...
Not yet exposed through the Haxe API: Python package modules, in-kernel stream-parallel lowering, NumPy/PyTorch import, GUI/window interop, and Python decorators.
