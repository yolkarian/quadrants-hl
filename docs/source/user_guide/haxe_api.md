...
`shape` and `dtype` are typed properties; `rank()`, `numel()`, and `shapeCopy()` provide stable method-style queries. Field parameters are direct SNode resources; fields are not mirrored through tensors on launch. Field placement supports core SNode domain offsets through the builder path:
