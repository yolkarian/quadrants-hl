...
class TestLocalReassignment {
  ...
  public static function run():Void {
    ...
    try {
      var input = ctx.ndarrayF32([F32_N]);
      var outF = ctx.ndarrayF32([F32_N]);
      var fValues = [-1.0, 0.25, 2.0, 7.0];
      var fExpected = [0.0, 0.25, 1.0, 1.0];
      for (i in 0...F32_N) input.writeF32(i, fValues[i]);
      ...
      ctx.sync();
      for (i in 0...F32_N) expectNear('f32_local_reassign[${i}]', outF.readF32(i), fExpected[i]);

      var pos = ctx.ndarrayF32([INT_N]);
      var outI = ctx.ndarrayI32([INT_N]);
      var posValues = [-2.0, -0.1, 0.0, 0.9, 1.0, 1.9, 2.0, 5.0];
      var intExpected = [0, 0, 0, 0, 1, 1, 2, 2];
      for (i in 0...INT_N) pos.writeF32(i, posValues[i]);

      var intClamp = Kernel.build(ctx, macro (pos:quadrants.Tensor<F32>, outI:quadrants.Tensor<Int>, n:Int, gridMin:F32, cellSize:F32, cellsX:Int) -> {
        for (i in 0...n) {
      ...
      ctx.sync();
      for (i in 0...INT_N) expectEq('int_conditional_reassign[${i}]', outI.readI32(i), intExpected[i]);

      var counts = ctx.ndarrayI32([COUNTER_N]);
      var neighbors = ctx.ndarrayI32([COUNTER_N * COUNTER_N]);
      counts.fillI32(-1);
      neighbors.fillI32(-1);
      var counter = Kernel.build(ctx, macro (counts:quadrants.Tensor<Int>, neighbors:quadrants.Tensor<Int>, n:Int, maxNeighbors:Int) -> {
        for (i in 0...n) {
          var count = 1;
          for (j in 0...n) {
      ...
      for (i in 0...COUNTER_N) {
        ...
        expectEq('nested_counter[${i}]', counts.readI32(i), expected);
        for (slot in 1...expected) {
          expectEq('nested_neighbor[${i},${slot}]', neighbors.readI32(i * COUNTER_N + slot), slot - 1);
        }
      }
    } catch (e:Dynamic) {
      for (kernel in kernels) kernel.close();
      ctx.close();
      throw e;
    }
    for (kernel in kernels) kernel.close();
