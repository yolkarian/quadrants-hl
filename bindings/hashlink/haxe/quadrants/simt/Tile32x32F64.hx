...
class Tile32x32F64 {
  @:qdFunc
  public static function loadTile32x32F64(tile:Tensor<F64>, source:Tensor<F64>, base:Int, stride:Int):Int {
  ...
  @:qdFunc
  public static function storeTile32x32F64(tile:Tensor<F64>, target:Tensor<F64>, base:Int, stride:Int):Int {
    var lane:Int = Block.threadIdx();
    if (lane < 1024) {
      var row:Int = lane >> 5;
      var col:Int = lane - (row << 5);
  ...
  @:qdFunc
  public static function transposeTile32x32F64(tile:Tensor<F64>):Int {
    var lane:Int = Block.threadIdx();
    if (lane < 1024) {
      var row:Int = lane >> 5;
      var col:Int = lane - (row << 5);
      if (row < col) {
        var a:F64 = tile.kernelRead(row * 32 + col);
        var b:F64 = tile.kernelRead(col * 32 + row);
  ...
  @:qdFunc
  public static function outerProductSubtractTile32x32F64(tile:Tensor<F64>, left:Tensor<F64>, right:Tensor<F64>, scale:F64):Int {
    var lane:Int = Block.threadIdx();
    if (lane < 1024) {
      var row:Int = lane >> 5;
      var col:Int = lane - (row << 5);
  ...
  @:qdFunc
  public static function choleskyTile32x32F64(tile:Tensor<F64>):Int {
    var lane:Int = Block.threadIdx();
    if (lane == 0) {
      var i:Int = 0;
      while (i < 32) {
        var j:Int = 0;
        while (j <= i) {
          var sum:F64 = tile.kernelRead(i * 32 + j);
          var k:Int = 0;
          while (k < j) {
  ...
  @:qdFunc
  public static function solveTriangularTile32x32F64(tile:Tensor<F64>, rhs:Tensor<F64>):Int {
    var lane:Int = Block.threadIdx();
    if (lane == 0) {
      var i:Int = 0;
      while (i < 32) {
