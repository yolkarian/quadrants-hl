...
class Tile16x16F32 {
  @:qdFunc
  public static function loadTile16x16F32(tile:Tensor<F32>, source:Tensor<F32>, base:Int, stride:Int):Int {
  ...
  @:qdFunc
  public static function storeTile16x16F32(tile:Tensor<F32>, target:Tensor<F32>, base:Int, stride:Int):Int {
    var lane:Int = Block.threadIdx();
    if (lane < 256) {
      var row:Int = lane >> 4;
      var col:Int = lane - (row << 4);
  ...
  @:qdFunc
  public static function transposeTile16x16F32(tile:Tensor<F32>):Int {
    var lane:Int = Block.threadIdx();
    if (lane < 256) {
      var row:Int = lane >> 4;
      var col:Int = lane - (row << 4);
      if (row < col) {
        var a:F32 = tile.kernelRead(row * 16 + col);
        var b:F32 = tile.kernelRead(col * 16 + row);
  ...
  @:qdFunc
  public static function outerProductSubtractTile16x16F32(tile:Tensor<F32>, left:Tensor<F32>, right:Tensor<F32>, scale:F32):Int {
    var lane:Int = Block.threadIdx();
    if (lane < 256) {
      var row:Int = lane >> 4;
      var col:Int = lane - (row << 4);
  ...
  @:qdFunc
  public static function choleskyTile16x16F32(tile:Tensor<F32>):Int {
    var lane:Int = Block.threadIdx();
    if (lane == 0) {
      var i:Int = 0;
      while (i < 16) {
        var j:Int = 0;
        while (j <= i) {
          var sum:F32 = tile.kernelRead(i * 16 + j);
          var k:Int = 0;
          while (k < j) {
  ...
  @:qdFunc
  public static function solveTriangularTile16x16F32(tile:Tensor<F32>, rhs:Tensor<F32>):Int {
    var lane:Int = Block.threadIdx();
    if (lane == 0) {
      var i:Int = 0;
      while (i < 16) {
